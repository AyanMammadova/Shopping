...
const cartcountdiv=document.getElementById("cartcountdiv")
let kod=''
let cartkod=''
let total=0
let totalcount=0
...
const products = [
 ...
 show()
function show(categor){
    kod=''
    products.forEach((item,i)=>{
        if(!categor || categor==item.category){
            kod+=`
            <div id="card${i}">
                <img src=${item.image} alt="">
                <h3>${item.name}</h3>
                <h4>Price:${item.price}$</h4>
                <button id="addbutton" onclick="add(${item.id})">Add to Cart</button>
            </div>
        `
        }
...
function add(index) {
  ...
  addedproducts.forEach((productid) => {
      ...
      cartkod += `
          <div id="addedproduct${productid}">
              <p>${product.name} - $ ${product.price} x <span id='count${productid}'>${quantities[productid]}</span> </p>
              <p class="spanclass">
                  <span onclick="minus(${productid})"><i class="fa-solid fa-minus"></i></span>
                  <span onclick="plus(${productid})"><i class="fa-solid fa-plus"></i></span>
                  <span onclick="deleteproduct(${productid})"><i class="fa-regular fa-trash-can"></i></span>
              </p>
          </div>
      `
      total += product.price * quantities[productid]
  })

  cartcountdiv.innerHTML = totalcount
  cartlist.innerHTML = cartkod
...
function deleteproduct(delid) {
  const indexofdelete = addedproducts.indexOf(delid);
  
  if (indexofdelete > -1) { 
      const product = products.find(item => item.id == delid)
      total -= product.price * quantities[delid]
      totalcount--
      cartcountdiv.innerHTML = totalcount;
      totalamount.innerHTML = `Total: ${total}$`

      addedproducts.splice(indexofdelete, 1)
      delete quantities[delid]

      const deletedproduct = document.getElementById(`addedproduct${delid}`)
      if (deletedproduct) {
...
function plus(plusid) {
  quantities[plusid] += 1
  const plusproduct = products.find(item => item.id == plusid)
  total += plusproduct.price

  document.getElementById(`count${plusid}`).innerText = quantities[plusid]
  totalamount.innerHTML = `Total: ${total}$`
}
function minus(minusid) {
  if (quantities[minusid] > 1) {
      quantities[minusid] -= 1;
      const product = products.find(item => item.id == minusid)
      total -= product.price

      document.getElementById(`count${minusid}`).innerText = quantities[minusid]
      totalamount.innerHTML = `Total: ${total}$`
  } else {
      deleteproduct(minusid)
  }
}
function showcart(){  
